...
1. **설치 확인**: `which works-cli` — 없으면 사용자에게 `pipx install .` (레포 clone 후) 안내하고 중단
2. **토큰 health check**: `works-cli whoami` — exit 2 + 401/403이면 PAT 미설정/만료. 사용자에게 안내:
   - `export WORKS_PAT='<your-pat>'` (env-var only)
   - **`config set-pat` 같은 명령은 없다**. 디스크 저장 지원 안 함.
3. **PAT 보안 가드레일**:
   - 토큰 평문을 메시지/로그/명령 출력에 절대 노출하지 않는다
   - `--verbose`/`-v` 옵션은 Authorization을 마스킹하므로 디버깅에 안전
   - 사용자에게 PAT를 받아도 다시 echo하지 않는다

## 표준 exit codes (셸 스크립팅 안정)

`0` ok / `1` generic / `2` auth(401/403) / `3` rate-limit(429) / `4` confirm(--yes 누락) / `5` usage
...
**모든 write 명령은 기본이 dry-run이다**. 호출 직전 payload를 stderr로 표시하고 exit 4로 종료한다. 실제 호출하려면 `--yes`를 추가한다.

| 명령 | 게이트 |
|------|--------|
| `mail send` | `--yes` + 외부 수신자 있으면 `--allow-external` + 동일 페이로드 10초 burst guard |
| `cal create` | `--yes` |
| `task create / complete / incomplete / delete` | `--yes` |
| `note create / delete` | `--yes` |
| `bot send / send-channel` | `--yes` |

**`mail read`는 기본이 본문 redact**. `--show-pii`를 명시해야 본문 노출. AI 에이전트가 본문을 사용자에게 보여주는 시나리오에서는 `--show-pii`를 의도적으로 켤지 한 번 확인.

## AI 에이전트가 따라야 할 절차

1. write 명령 실행 전, 사용자에게 **payload를 한 번 확인받는다**. dry-run으로 한 번 돌려 stderr 출력을 사용자에게 보여주고 OK 받은 뒤 `--yes` 추가.
2. `mail send`는 수신자 도메인을 살펴 외부면 `--allow-external` 필요함을 미리 안내.
3. 동일 메일을 짧은 시간 안에 두 번 보내려고 하면 burst guard에서 막힐 수 있다. 의도된 재발송이면 10초 대기 후 재시도.
