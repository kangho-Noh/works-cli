...
| 사용자 식별자 | `me` self-alias만 사용, 타 사용자 호출 불가 |
| **Write 명령** | **기본 dry-run, `--yes`로만 실제 호출** (mail send / cal create / task create·complete·incomplete·delete / note create·delete / bot send·send-channel) |
| **`mail send` 외부 게이트** | 본인/INTERNAL_DOMAINS 외 수신자 있으면 `--allow-external` 필요 |
| **`mail send` burst guard** | 동일 페이로드(to+subject+body) 10초 내 재발송 차단 (`~/.works-cli/last-send`, mode 0600) |
| **`mail read` --show-pii** | 본문은 기본 redact, `--show-pii`로만 노출 |
| Exit codes | `0` ok / `1` generic / `2` auth(401/403) / `3` rate-limit(429) / `4` confirm(--yes 누락/external/burst) / `5` usage |
...
works-cli mail read <mailId>                          # 본문 redact
works-cli mail read <mailId> --show-pii               # 본문 노출

# 발송 — 기본 dry-run. 실제 발송은 --yes 필요.
works-cli mail send --to me@inside.com --subject hi --body world           # dry-run, exit 4
works-cli mail send --to me@inside.com --subject hi --body world --yes     # 사내 수신자 발송
works-cli mail send --to ext@outside.com --subject hi --body world --yes --allow-external
works-cli mail send --to a@x.com --subject hi --payload @/tmp/mail.json --yes --allow-external
```
...
works-cli cal events --from 2026-05-28 --to 2026-05-28
works-cli cal events --from 2026-05-28 --to 2026-05-28 --expand   # 오늘 인스턴스만 (반복 일정 펴기)
works-cli cal events --calendar <calId> --from 2026-05-28 --to 2026-05-29
works-cli cal show <eventId>
# 생성 — 기본 dry-run, --yes로만 실제 호출
works-cli cal create --summary "회의" --start 2026-05-29T10:00:00 --end 2026-05-29T11:00:00 --yes
works-cli cal create --summary "회의" --start ... --end ... --attendees a@x.com,b@x.com --yes
```
...
# Write — 기본 dry-run
works-cli bot send --bot <botId> --user user@yourdomain.com --message "안녕" --yes
works-cli bot send-channel --bot <botId> --channel <chId> --message "공지" --yes
works-cli bot send --bot <botId> --user u@x.com --payload @/tmp/sticker.json --yes
```
...
# Write 명령 — 기본 dry-run, --yes로만 실행
works-cli task create --title "백업 점검" --due 2026-05-30T18:00:00+09:00 --yes
works-cli task complete <taskId> --yes
works-cli task incomplete <taskId> --yes
works-cli task delete <taskId> --yes
```
...
# Write — 기본 dry-run
works-cli note create --group <groupId> --title T --body B --yes
works-cli note delete <postId> --group <groupId> --yes
```
